...
export const Sidebar: React.FC = () => {
  const { state, currentTab, setCurrentTab, activeView, setActiveView, setIsSettingsOpen, exportBackup } = useApp();
  ...
  const navItems = [
    ...
    { id: 'tab-cong-viec', title: 'CÔNG VIỆC & KINH DOANH', icon: Briefcase, count: getBadgeCount('tab-cong-viec'), color: 'text-blue-400' },
    { id: 'tab-doi-song', title: 'ĐỜI SỐNG & GIA ĐÌNH', icon: Heart, count: getBadgeCount('tab-doi-song'), color: 'text-rose-400' },
    { id: 'tab-nhan-su-ai', title: 'ĐỘI NGŨ NHÂN SỰ AI', icon: Bot, count: getBadgeCount('tab-nhan-su-ai'), color: 'text-emerald-400', isAi: true },
    { id: 'tab-tat-ca', title: 'TẤT CẢ CÔNG CỤ', icon: LayoutGrid, count: getBadgeCount('tab-tat-ca'), color: 'text-sky-400' }
  ];

  return (
    <>
      <aside className="flex flex-col w-72 bg-[#0a1128] border-r border-slate-800/80 min-h-screen p-4 select-none shrink-0 z-20">
          ...
          <div className="bg-[#0e1738] rounded-[15px] p-3 flex items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="flex items-center gap-2.5 mt-1.5 pt-1.5 border-t border-white/10 text-[11px] text-slate-400">
                <button onClick={() => setIsSettingsOpen(true)} className="flex items-center gap-1 hover:text-sky-300 transition-colors">
        ...
        <div className="space-y-2 mb-4">
          ...
          <button 
            onClick={() => {
              alert('Để cài đặt PWA: Nhấn vào biểu tượng Cài đặt trên thanh địa chỉ trình duyệt hoặc chạy Mo-App.bat');
            }}
        ...
        <nav className="flex-1 flex flex-col gap-1.5 overflow-y-auto pr-1">
          <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider px-2 py-1">Danh Mục Quản Trị</div>
            ...
            return (
              <button
                ...
                className={`flex items-center justify-between px-3 py-2.5 rounded-xl text-xs font-medium transition-all ${
                  isActive 
                    ? 'bg-sky-500/20 border border-sky-500/40 text-white shadow-sm shadow-sky-500/15' 
              ...
              >
                <div className="flex items-center gap-2.5 min-w-0">
                  ...
                  {item.isAi && <Sparkles className="w-3 h-3 text-amber-400 shrink-0 animate-spin" style={{ animationDuration: '6s' }} />}
                </div>
        ...
        <div className="pt-3 border-t border-white/5 text-[11px] text-slate-400 flex items-center justify-between">
          <span>Ngôi Nhà Số Enterprise</span>
